...
노션에 `/embed` 로 넣어 쓰는 질문·답변 위젯 세 개와 공용 파일입니다. 데이터는 Cloudflare Worker(`worker.js`)를 거쳐 D1에 저장되고(이미지 포함), 등록된 접근 키로만 읽고 쓸 수 있습니다.
...
| `compose.html` | 미니 질문형. 가이드 페이지 하단의 한 줄 등록 위젯(카테고리는 위젯에서 선택) |
| `index-pink.html`, `add-pink.html`, `compose-pink.html` | 위 셋의 색만 바꾼 사본(보라 → 핑크). 구조·문구·동작과 저장되는 데이터는 같습니다 |
...
| 미니 질문형(가이드 페이지 하단) | `https://kjm9954.github.io/my-notion-widget/handover-qa/compose.html#k=<접근 키>` (모든 가이드 페이지에 같은 주소) | 320 |
| 예전 입력 전용(가이드 페이지 하단) | `https://kjm9954.github.io/my-notion-widget/handover-qa/index.html?mode=compose&cat1=<카테고리>&guide=<페이지 이름>#k=<접근 키>` | 96~268 |
...
**색**: 파일명에 `-pink` 를 붙이면 같은 위젯의 핑크 사본이 열립니다(예: `index-pink.html#k=<접근 키>`). 같은 방·같은 데이터를 쓰므로 페이지마다 색을 섞어 써도 됩니다.
...
| 위젯 | 파일 | 놓이는 곳 | 역할 |
|---|---|---|---|
| 목록형 | `index.html` | 메인 페이지 오른쪽 | 검색·목록·스레드·답변·상태 변경 |
| 질문형 | `add.html` | 메인 페이지 오른쪽, 목록형 바로 위 | 질문 등록(카테고리 직접 선택) |
| 미니 질문형 | `compose.html` | 각 가이드 페이지 하단 | 질문 등록(카테고리를 하단 줄에서 선택) |

- 셋 모두 같은 접근 키, 같은 방 설정, 같은 작성자 선택(`localStorage["qa.author"]`)을 씁니다. 어디서 등록해도 목록형에 나타납니다.
- 미니 질문형은 모든 가이드 페이지에 같은 주소로 넣습니다. 카테고리는 펼친 폼의 하단 줄에서 고르고, 마지막으로 고른 카테고리를 기억합니다(질문형과 같은 `localStorage["qa.lastCategory"]`). 주소에 `&major=<카테고리>` 를 붙이면 처음 선택값으로, `&guide=<가이드명>` 을 붙이면 질문에 함께 저장됩니다(선택 사항).
- 미니 질문형은 평소 한 줄(40px)로 접혀 있고, 누르면 폼이 펼쳐집니다. 노션 임베드 높이는 **320**으로 넣고, 접혀 있을 때 남는 공간은 비워 둡니다. 내용이나 펼친 스레드가 그보다 길면 그 칸 안에서만 스크롤합니다.
- 미니 질문형의 "지난 질문 N건"은 방 전체 질문 수입니다(`guide` 를 붙였다면 그 가이드의 질문 수). 작성 중 내용은 `localStorage["qa.draft.mini"]`(가이드명을 붙였다면 `qa.draft.<가이드명>`)에 임시 저장됩니다.
...
- 목록형은 고른 작성자의 역할에 따라 다르게 보입니다. 답하는 사람에게는 **답을 기다리는 질문**을 맨 위에 모으고 카드에 마지막 글을 보여줍니다. 묻는 사람에게는 **아직 답이 오지 않은 자기 질문**을 맨 위에 모으고, 카드에 상대의 답 대신 자기가 쓴 질문을 보여줍니다. 새 답이 온 질문은 아래 목록에서 `NEW` 배지로 표시합니다. 맨 위 묶음이 비면 나머지 목록에는 제목을 붙이지 않습니다.
