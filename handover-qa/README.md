...
| `compose.html` | 미니 질문형. 가이드 페이지 하단의 한 줄 등록 위젯(카테고리·가이드명 자동) |
...
| 미니 질문형(가이드 페이지 하단) | `https://kjm9954.github.io/my-notion-widget/handover-qa/compose.html#k=<접근 키>&major=<카테고리>&guide=<가이드명>` | 320 |
...
| 미니 질문형 | `compose.html` | 각 가이드 페이지 하단 | 질문 등록(카테고리·가이드명 자동) |

- 셋 모두 같은 접근 키, 같은 방 설정, 같은 작성자 선택(`localStorage["qa.author"]`)을 씁니다. 어디서 등록해도 목록형에 나타납니다.
- 미니 질문형의 `major` 는 방 설정의 카테고리 이름과 같아야 합니다. 다르면 위젯에 주소를 확인하라는 한 줄이 뜹니다. `major`·`guide` 는 화면에서 바꿀 수 없습니다.
- 미니 질문형은 평소 한 줄(40px)로 접혀 있고, 누르면 폼이 펼쳐집니다. 노션 임베드 높이는 **320**으로 넣고, 접혀 있을 때 남는 공간은 비워 둡니다. 내용이나 펼친 스레드가 그보다 길면 그 칸 안에서만 스크롤합니다.
- 미니 질문형의 "지난 질문 N건"은 같은 `guide` 로 등록된 질문 수입니다(`guide` 가 없으면 같은 카테고리). 작성 중 내용은 가이드별로 `localStorage["qa.draft.<가이드명>"]` 에 임시 저장됩니다.
