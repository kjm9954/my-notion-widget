...
- 빨강 계열·경고색은 파괴적 동작(삭제 버튼)에만. 낮은 값·공백·안 쓴 날에 쓰지 마라.
...
- 빈 상태 릴레이는 기존 점선과 같은 자리에 공통 점선 하나만 그린다. 점선을 두 겹으로 겹치지 마라.
...
- 삭제 제어는 자리 이동 없이 유지하며 파괴적 색은 `#A56B6B`만 쓴다.
...
```
- 모서리 손잡이는 **비율 유지** 확대/축소: 스케일 `k` 하나로 폭·높이를 함께 바꾼다.
- 좌우 4×46 손잡이는 **가로만** 바꾼다. 카드의 레이아웃 폭을 다시 계산하되 글자·아이콘·칸을 가로로 늘여 그리지 않고, 현재 세로 크기와 `k`는 유지한다.
- 위아래 46×4 손잡이는 **세로만** 바꾼다. 현재 가로 크기와 `k`는 유지하며, 목록 위젯은 목록 영역 높이로 반영한다.
- 축별 손잡이로 바꾼 폭·높이에서도 흰 프레임은 실제 카드 외곽과 정확히 맞아야 한다. 빈 여백 띠나 비균일 `transform:scaleX/scaleY`를 만들지 마라.
- 임베드가 숨겨져 있거나 화면 밖이면 `requestAnimationFrame`이 아예 돌지 않는다. 첫 배치는 동기로 커밋하고, 이후 갱신도 rAF와 타이머 중 먼저 오는 쪽으로 반드시 반영한다.
- 범위: 모서리 스케일은 최소 `0.08`, 가로는 `max(120px, 기본폭×0.3)`, 세로는 `max(48px, 기본높이×0.2)`부터 현재 `window.innerWidth/innerHeight`까지. 임베드 밖으로 넘쳐 잘리면 안 된다.
- 드래그 중 핸들 위에 현재 크기를 9px `#A9BAC2`로 표시(`940 × 620`).
- 목록/격자가 주인 위젯은 **카드 아래 가운데 46×4 손잡이**(radius 2, 같은 색 규칙)를 추가 — 끌면 그 영역 높이만 늘어난다(비율 무관).
- 저장: `widget-size-<파일명>` → `{ scale, contentW, frameH }` 또는 목록 위젯의 `{ scale, contentW, frameH, listH }`와 각 잠금 상태. 로드 시 축별 크기를 복원하고 최대치를 넘으면 자동으로 낮춘다. 예전에 저장된 `{ width, height }`도 스케일로 환산해 읽는다.
- 가로 폭을 맞춰야 하는 위젯 묶음은 같은 `data-widget-width-key`를 쓴다. 이 키에는 `{ scale, visualW }`만 공유하고, 높이 때문에 실제 스케일이 달라질 때는 논리 `contentW`를 보정해 바깥 가로 폭을 맞춘다. `frameH/listH`는 각 위젯의 `data-widget-key`에 따로 저장해 세로 높이가 섞이지 않게 한다.
...
- **막대 게이지는 `today.html`의 HP와 `stats.html`의 기존 HP/MP만 허용한다.** 다른 위젯에 진행 막대를 추가하지 마라(진행 상황은 텍스트로).
...
아래 폭 기반 크기 조절·재배치는 `any-pointer: coarse`인 모바일·태블릿에서만 적용한다. 데스크톱은 임베드 가로 폭이 바뀌어도 저장된 위젯 크기와 데스크톱 배치를 유지한다.
...
- [ ] hover 전 핸들이 완전히 안 보이는가 / 모서리는 비율 유지, 좌우는 가로만, 위아래는 세로만 변하는가
...
화면(상세·모달·패널·배너)을 닫는 버튼은 헤더 우상단 ✕ 하나(`aria-label="닫기"`). 텍스트 닫기·← 목록 금지.
항목을 지우는 ✕는 행 오른쪽 끝, 행의 세로 중앙(체크박스와 같은 높이). 한 번 누르면 바로 삭제 — 확인 단계·"삭제할까요?" 금지.
기호는 ✕(U+2715)만. 크기 데스크톱 28×28(행 hover 시 표시), 터치 44×44 항상 표시 `#C6D6DD`.
폼 안의 취소·저장·삭제만 텍스트 버튼, 하단 우측 정렬.
