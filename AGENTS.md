...
수정 진입·상태 변경·삭제는 한 번의 클릭/탭으로 실행한다. 행의 프로젝트·Q는 클릭하면 기존 선택 목록을 열고, 원하는 항목을 선택할 때만 저장한다(클릭마다 다음 값으로 순환하지 않는다). 진행 상태는 기존 순환 동작을 유지한다.
아이폰 전용 화면은 `is-widget-mobile`, 일반 재배치는 `is-widget-reflow`에서만 켠다. iPad에서는 저장된 논리 배치에 비율 축소만 적용하며, 모바일·태블릿에서 `widget-size-*` 및 공통 폭 키를 쓰지 않는다.
